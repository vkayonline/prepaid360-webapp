...
const statusVariantMap: {
    [key: string]: "default" | "secondary" | "destructive" | "outline" | "success" | "warning"
} = {
    "SUCCESS": "success",
    "PENDING": "warning",
    "FAILED": "destructive",
...
export default function BatchDetailsPage() {
    ...
    useEffect(() => {
        ...
        const fetchApplications = async (page = 0, size = 10) => {
            setIsLoading(true)
            try {
                const data = await listApplicationsInBatch(parseInt(batchId), page, size);
    ...
    const handleApprove = async () => {
        try {
            await approveOrRejectBatch(Number(batchId), "APPROVE", "All applications approved");
            handleDialogClose();
                ...
                <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        <Show permission="applications.approve">
                            <Button onClick={() => openConfirmationDialog("approve")} variant="success"
                                    className="w-32">
                        ...
                        <Show permission="applications.reject">
                            <Button onClick={() => openConfirmationDialog("reject")} variant="destructive"
                                    className="w-32">
                                        ...
                                        applications.map((app) => (
                                            <TableRow key={app.id}>
                                                <TableCell className="font-medium">
                                                    <Link to={`/applications/${batchId}/${app.id}`}
